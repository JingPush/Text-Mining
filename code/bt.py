...
Class BaggedTree():
    ...
    def pred_BT(self, X, num, max_depth, min_size):
        pred_total = np.zeros((len(test), num))
        pred_final = np.zeros((len(test), 1))
        for i in range(num):
            #new_X = [random.choice(X) for j in range(X.shape[0])]
            new_train = [random.choice(train) for j in range(len(train))]
            tree = build_tree(self, new_train, max_depth, min_size)
...
if __name__ == '__main__':
	BTree_model = BaggedTree()
	#BTree_model.build_tree(self, dataset, max_depth, min_size)
	y_hat = BTree_model.pred_BT(self, X, num, max_depth, min_size)
